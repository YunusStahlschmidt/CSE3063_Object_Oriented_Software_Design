        ...
        if (continueRandom == null) {
            ...
            for (Instance anInstance : dataset.getInstances()) {
                ...
                for (int i = 0; i < numberOfAssignmentsPerInstance; i++) {
                    ...
                    if (newLabelAssignment.getAssignedLabelId().size() == 1) {
                        ...
                        logger.info("user id:" + currentUser.getId() + " " + currentUser.getName()
                                + " tagged instance id:" + anInstance.getId() + " with class label "
                                + newLabelAssignment.getAssignedLabelId() + ":" + label_name + " instance: "
                        ...
                        for (int n = 0; n < newLabelAssignment.getAssignedLabelId().size(); n++) {
                            labels.add(dataset.getLabels()
                                    .get((int) newLabelAssignment.getSpecificAssignedLabelId(n) - 1).getLabelText());
                        }
                        logger.info("user id:" + currentUser.getId() + " " + currentUser.getName()
                                + " tagged instance id:" + anInstance.getId() + " with class labels "
            ...
            for (Instance anInstance : dataset.getInstances()) {
                ...
                while (running) {
                    ...
                    System.out.printf(
                            "\nChoose max %s label/s number (seperate with coma if you choose more than one): ",
                            maxLabel);

                    String assign = sc.nextLine().trim();

                    if (assign.equals("")) {
                        System.out.println("Choose a label. Please try again.");
                        continue;
                    }
                    chosen = Arrays.asList(assign.split(","));

                    if (chosen.size() > maxLabel)
                        System.out.println("\nOver maximum value. Delete one or more labels\n");
                        ...
                        for (int i = 0; i < chosen.size(); i++) {
                            Label tempLabel = allLabels.get(Integer.parseInt(chosen.get(i)));
                            addedLabels.add(tempLabel);
