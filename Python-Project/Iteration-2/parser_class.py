...
class Parser:
    ...
    def parse_poll_reports(self):
        ...
        for f in poll_files:
            ...
            csv_file = pd.read_csv(os.path.join(self._POLL_PATH, f), names=list(range(30)))
            csv_file = csv_file.iloc[3:, 1:-1]
            poll_date = csv_file.values[0][1].split()
            poll_date = '_'.join(poll_date[0].split('-')) + '_' + '_'.join(poll_date[1].split(':'))
            csv_file = csv_file.iloc[3:, :].dropna(axis=1, how='all')

            handled_poll_objects = set()
            for row in csv_file.values:
                text = ' '.join(row[3].split())
                quest_obj = self._question_list[text]
                for poll_obj in reversed(self._polls):
                    if quest_obj in poll_obj._question_list:
                        break

                if poll_obj._is_done:
                    continue

                if poll_obj._date != poll_date:
                    poll_obj._date = poll_date
                ...
                handled_poll_objects.add(poll_obj)    
            self._anomalies.append(anomalies_per_poll)
            for poll_ in handled_poll_objects:
                poll_._is_done = True
